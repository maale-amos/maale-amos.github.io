...
export async function handleKlitaRegister(request, env) {
  ...
  if (!EMAIL_RE.test(email)) return error(400, 'bad_email', env);
  if (password.length < 10) return error(400, 'weak_password', env, 'סיסמה חייבת להיות באורך 10 תווים לפחות');
...
export async function handleKlitaFormSave(request, env) {
  ...
  if ('__proto__' in body || 'constructor' in body || 'prototype' in body) return error(400, 'bad_json', env);
  ...
  // Also update the auto-fill fields on the applicant record from the form
  // whenever those fields are present. Users typing name/id/phone in the form
  // shouldn't have to re-enter them next time.
  ...
  let formId;
  if (body.form_id) {
    // Update existing draft owned by this applicant
    const own = await env.DB.prepare('SELECT id FROM application_forms WHERE id=? AND applicant_id=?')
      .bind(Number(body.form_id), applicant.id).first();
...
export async function handleKlitaFormGet(request, env, formId) {
  ...
  if (!Number.isFinite(id) || id <= 0) return error(400, 'bad_id', env);
  ...
  if (!row) {
    // committee / admin can read any form
